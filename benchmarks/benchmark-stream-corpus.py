#!/usr/bin/env python3
"""Generate a deterministic macOS ``say`` corpus and benchmark Kestrel streaming."""
...
PRESETS = (
    Preset("current-160-480-4000-s2", 160.0, 480.0, 4000.0, 2),
    Preset("candidate-160-320-2000-s2", 160.0, 320.0, 2000.0, 2),
)
...
def main() -> int:
    ...
    with exclusive_mps_lock():
        ...
        try:
            ...
            for item_index, item in enumerate(CORPUS):
                ...
                order = PRESETS if item_index % 2 == 0 else tuple(reversed(PRESETS))
    ...
    report = {
        "experiment": "macos-say-streaming-corpus-current-vs-candidate-apple-mps",
        "versions": {
        ...
        "metric_notes": [
            ...
            "Preset order alternates by utterance; one warm model is shared; the MPS lock covers model load and all runs.",
        ],
    }
