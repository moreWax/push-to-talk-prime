...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="ptt-real-tui-") as temp:
        ...
        subprocess.run(["ffmpeg","-y","-loglevel","error","-i",str(video),"-i",str(ASSETS/"demo-input.wav"),"-filter_complex",f"[1:a]adelay={delay}|{delay},apad=pad_dur={duration}[a]","-map","0:v","-map","[a]","-c:v","copy","-c:a","aac","-shortest",str(ASSETS/"prime-live-demo.mp4")],check=True)
        palette=temp/"palette.png";subprocess.run(["ffmpeg","-y","-loglevel","error","-i",str(video),"-vf","fps=10,scale=800:-1:flags=lanczos,palettegen=max_colors=96",str(palette)],check=True);subprocess.run(["ffmpeg","-y","-loglevel","error","-i",str(video),"-i",str(palette),"-lavfi","fps=10,scale=800:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer",str(ASSETS/"prime-live-demo.gif")],check=True)
    print(ASSETS/"prime-live-demo.mp4");print(ASSETS/"prime-live-demo.gif")
